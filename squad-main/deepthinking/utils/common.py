...
from .noisy_cifar_data import prepare_image_loader
...
def get_dataloaders(args):
    if args.problem == 'prefix_sums':
        ...
        return prepare_chess_loader(train_batch_size=args.train_batch_size,
                                                                      ...
                                                                      train_data=args.train_data,
                                                                      test_data=args.test_data)
    elif args.problem == 'denoise':
        return prepare_image_loader(train_batch_size=args.train_batch_size,
                                                                      test_batch_size=args.test_batch_size,
                                                                      train_data=args.train_data,
                                                                      test_data=args.test_data)
    else:
...
def get_model(model, width, max_iters, in_channels=3, out_channels=2):
    model = model.lower()
    net = getattr(models, model)(width=width, in_channels=in_channels, max_iters=max_iters, out_channels=out_channels)
    return net
...
def load_model_from_checkpoint(model, model_path, width, problem, max_iters, device):
    ...
    if problem == "denoise":
        out_channels = 3
    print(problem, out_channels)

    net = get_model(model, width, in_channels=in_channels, max_iters=max_iters, out_channels=out_channels)
    net = net.to(device)
    if device == "cuda":
